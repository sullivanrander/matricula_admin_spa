    ...
    <v-col cols="12" sm="12" md="12" lg="12">
      <v-form ref="form" v-model="valid">
        <v-row dense>
          ...
          <v-col cols="12" sm="12" md="12">
            <course-list v-model="coursesSelected" enable-select single-select :selected="[registration.course]"/>
          </v-col>

          <v-col cols="12" sm="12" md="12" class="text-right pt-5">
            <v-btn text class="mr-1" @click="$refs.form.reset()">Limpar</v-btn>
            <v-btn color="primary" @click="updateRegistration()">Alterar</v-btn>
          </v-col>
        </v-row>
      </v-form>
    </v-col>
    ...
    methods: {
      index() {
        this.$store.dispatch('fetchRegistration', {
          registration_id: this.$route.params.id
        }).then((response) => {
      ...
      updateRegistration() {
        ...
        this.$store.dispatch('updateRegistration', {
          registration_id: this.$route.params.id,
